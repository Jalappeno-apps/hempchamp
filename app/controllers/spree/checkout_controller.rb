  ...
  class CheckoutController < Spree::StoreController
    ...
    def update
      ...
      if params["order"]["payments_attributes"]
        pm = Spree::PaymentMethod.find(params["order"]["payments_attributes"][0]["payment_method_id"])

        if (pm.is_a?(Spree::PaymentMethod::Payu))
          redirect_path = PayuCreateOrder.execute!(
            @order.number,
      ...
      if @order.update_from_params(params, permitted_checkout_attributes, request.headers.env)
        ...
        if @order.completed?
          ...
          if redirect_path != ""
            redirect_to completion_route(custom_params = { payu: redirect_path })
          else
    ...
    private
      ...
      def completion_route(custom_params = nil)
        return custom_params[:payu] if custom_params&.key? :payu
